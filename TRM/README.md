...
#### 公式（7）：
$$\min\limits_{q^{*},p^{*}}\sum\limits_{(u,i)\in K}(r_{u,i}-q^{T}_{i}p_{u})^{2}+\lambda(||q_{i}||^{2}+||p_{u}||^{2})，为了防止过拟合，加入正则项$$
...
$$\min\limits_{q^{*},p^{*}}\sum\limits_{(u,i)\in K}(r_{u,i}-\mu-b_{i}-b_{u}-q^{T}_{i}p_{u})^{2}+\lambda(||q_{i}||^{2}+||p_{u}||^{2}+b_{i}^{2}+b_{u}^{2})$$
